# Mimir App Audit — Security checklist
...
detail, and the specific remediation (the check text describes the fix). Mark genuinely
inapplicable checks N/A with a one-line reason. Fixes happen only after the user opts in
(see SKILL.md "App audit mode") on a `mimir/security` branch, never inline.

Severity shown per check is the default for that class; raise or lower it for the
specific instance you find. This file defers committed-secret and dependency-vuln checks
to Mimir's environment audit where noted, to avoid double-reporting.


### 1. Hash passwords with a modern algorithm  ·  **severity: critical**
...
### 2. Enforce strong password requirements  ·  **severity: high**
...
### 3. Rate limit login attempts  ·  **severity: high**
...
### 4. Lock accounts after repeated failures  ·  **severity: high**
...
### 5. Stop username and email enumeration  ·  **severity: high**
...
### 6. Secure the password reset flow  ·  **severity: high**
...
### 7. Regenerate sessions to block fixation  ·  **severity: high**
...
### 8. Add multi-factor authentication support  ·  **severity: high**
...
### 10. Harden the remember-me feature  ·  **severity: high**
...
### 11. Add bot protection to forms  ·  **severity: high**
...
### 12. Throttle password reset and verification emails  ·  **severity: high**
...
### 13. Set secure session cookie flags  ·  **severity: high**
...
### 14. Add idle and absolute session timeouts  ·  **severity: high**
...
### 15. Fully invalidate sessions on logout  ·  **severity: high**
...
### 16. Validate every JWT claim properly  ·  **severity: high**
...
### 17. Block JWT algorithm confusion attacks  ·  **severity: high**
...
### 18. Strengthen and rotate signing secrets  ·  **severity: high**
...
### 19. Store auth tokens safely client-side  ·  **severity: high**
...
### 20. Issue short-lived access tokens  ·  **severity: high**
...
### 21. Rotate refresh tokens on use  ·  **severity: high**
...
### 22. Build a token revocation mechanism  ·  **severity: high**
...
### 23. Enforce authorization on the server  ·  **severity: critical**
...
### 24. Fix insecure direct object references  ·  **severity: critical**
...
### 25. Implement role-based access control  ·  **severity: critical**
...
### 26. Apply a default-deny access policy  ·  **severity: critical**
...
### 27. Lock down all admin routes  ·  **severity: critical**
...
### 28. Close privilege escalation paths  ·  **severity: critical**
...
### 29. Isolate data between tenants  ·  **severity: critical**
...
### 30. Add function-level permission checks  ·  **severity: critical**
...
### 31. Block mass assignment of fields  ·  **severity: critical**
...
### 32. Re-verify access on each request  ·  **severity: critical**
...
### 33. Use parameterized database queries  ·  **severity: critical**
...
### 34. Prevent NoSQL query injection  ·  **severity: critical**
...
### 35. Block operating system command injection  ·  **severity: critical**
...
### 36. Close ORM-level injection vectors  ·  **severity: critical**
...
### 37. Sanitize inputs used in LDAP  ·  **severity: critical**
...
### 38. Escape identifiers in dynamic queries  ·  **severity: critical**
...
### 39. Validate and bound user regex  ·  **severity: critical**
...
### 40. Prevent prototype pollution in objects  ·  **severity: critical**
...
### 41. Disable XML external entity parsing  ·  **severity: critical**
...
### 42. Prevent formula injection in exports  ·  **severity: critical**
...
### 43. Encode output to stop XSS  ·  **severity: high**
...
### 44. Sanitize user-submitted HTML content  ·  **severity: high**
...
### 45. Add a Content Security Policy  ·  **severity: high**
...
### 46. Eliminate DOM-based XSS sinks  ·  **severity: high**
...
### 47. Prevent stored XSS in content  ·  **severity: high**
...
### 48. Block server-side template injection  ·  **severity: high**
...
### 49. Audit unsafe HTML render bypasses  ·  **severity: high**
...
### 50. Sanitize content rendered into emails  ·  **severity: high**
...
### 51. Add CSRF protection to mutations  ·  **severity: high**
...
### 52. Configure CORS without dangerous wildcards  ·  **severity: high**
...
### 53. Verify Origin and Referer headers  ·  **severity: high**
...
### 54. Set the SameSite cookie attribute  ·  **severity: high**
...
### 55. Block open redirect vulnerabilities  ·  **severity: high**
...
### 56. Prevent clickjacking with frame controls  ·  **severity: high**
...
### 57. Validate OAuth redirect URIs strictly  ·  **severity: high**
...
### 58. Validate input on the server  ·  **severity: high**
...
### 59. Validate types, lengths, and formats  ·  **severity: high**
...
### 60. Cap request body and payload size  ·  **severity: high**
...
### 61. Block path traversal in file access  ·  **severity: high**
...
### 62. Prevent server-side request forgery  ·  **severity: high**
...
### 63. Stop HTTP header and CRLF injection  ·  **severity: high**
...
### 64. Enforce strict Content-Type validation  ·  **severity: high**
...
### 65. Reject malformed and unexpected fields  ·  **severity: high**
...
### 66. Move hardcoded secrets to environment variables  ·  **severity: critical**
...
> **Dedup note:** Mimir's environment audit (`/mimir`) already scans tracked files for committed secrets via its `secrets` check. Run that for machine-wide coverage. Here, focus on secrets hardcoded in THIS project's source that the environment scan's patterns might miss.

### 67. Remove committed secrets from git history  ·  **severity: critical**
...
> **Dedup note:** Mimir's environment audit (`/mimir`) flags secrets in tracked files and git history. Defer history-purge guidance and rotation to that finding; do not duplicate it.

### 68. Keep secrets out of client bundles  ·  **severity: critical**
...
### 69. Separate development and production credentials  ·  **severity: critical**
...
### 70. Rotate any exposed or leaked secrets  ·  **severity: critical**
...
### 71. Hide stack traces in production  ·  **severity: critical**
...
### 72. Remove revealing server response headers  ·  **severity: critical**
...
### 73. Disable directory listing and indexes  ·  **severity: critical**
...
### 74. Harden insecure default framework settings  ·  **severity: critical**
...
### 75. Force HTTPS across the application  ·  **severity: high**
...
### 76. Enable HSTS to enforce TLS  ·  **severity: high**
...
### 77. Encrypt sensitive data at rest  ·  **severity: high**
...
### 78. Replace weak cryptographic algorithms  ·  **severity: high**
...
### 79. Use a cryptographically secure random generator  ·  **severity: high**
...
### 80. Hash security tokens before storage  ·  **severity: high**
...
### 81. Mask sensitive values in logs  ·  **severity: high**
...
### 82. Encrypt stored personal identifiable information  ·  **severity: high**
...
### 83. Prevent caching of sensitive pages  ·  **severity: high**
...
### 84. Add global API rate limiting  ·  **severity: medium**
...
### 85. Limit GraphQL query depth and cost  ·  **severity: medium**
...
### 86. Disable GraphQL introspection in production  ·  **severity: medium**
...
### 87. Trim over-exposed fields in responses  ·  **severity: medium**
...
### 88. Enforce maximum pagination page sizes  ·  **severity: medium**
...
### 89. Verify signatures on incoming webhooks  ·  **severity: medium**
...
### 90. Add idempotency keys to mutations  ·  **severity: medium**
...
### 91. Validate upload types by content  ·  **severity: high**
...
### 92. Enforce a maximum upload size  ·  **severity: high**
...
### 93. Store uploads outside the webroot  ·  **severity: high**
...
### 94. Randomize and sanitize stored filenames  ·  **severity: high**
...
### 95. Serve user files without execution  ·  **severity: high**
...
### 96. Audit dependencies for known vulnerabilities  ·  **severity: medium**
...
> **Dedup note:** Mimir's environment audit (`/mimir`) runs `npm audit` via its `supply` check. Run that for the authoritative dependency-vulnerability scan rather than repeating it here.

### 97. Lock dependency versions with a lockfile  ·  **severity: medium**
...
> **Dedup note:** Mimir's environment audit (`/mimir`) checks for missing lockfiles via its `supply` check. Defer to it.

### 98. Add Subresource Integrity to scripts  ·  **severity: medium**
...
### 99. Log security events for monitoring  ·  **severity: medium**
