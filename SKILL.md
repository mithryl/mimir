...
3. **Load the checklist** for the mode from `reference/app_checks/<security|performance|ui>.md`. Each file holds roughly 100 checks, each with a default severity and inline dedup notes.
4. **Review, do not edit.** Work through the checklist against the project's source. Record a finding only where the issue is genuinely present: severity (adjust the per-class default to the actual instance), short title, `file:line`, one-line detail, and the specific remediation from the check text. Mark inapplicable checks N/A with a one-line reason. Never assume a framework or platform default satisfies a check — verify it in this repo. If you cannot find evidence either way, record the check as **UNKNOWN** with what you would need to see; an unproven pass is not a pass. Honor the dedup notes — do not re-report what another mode or the environment audit already owns.
...
6. **Present** the report in this order: a scoreboard line up top ("Passed X / Y applicable · Z findings · N/A: a · Unknown: b" — Y counts applicable checks only), then the prioritized severity table (same Output Shape as the environment audit), then a short UNKNOWN list (what couldn't be determined and what evidence would settle it), then — for security mode — the checklist's "Ask the user" questions as open items, unscored. Close with **Fix first:** the three highest-impact findings in order, one line each on why they outrank the rest.
